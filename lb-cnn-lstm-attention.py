...
alexa1000000_path = "E:\\论文\\冷宝\\Reproduct-latest-DGA-tasks-main\\Code\\w.csv"
...
class Reines(nn.Module):
    def __init__(self):
        ...
        self.embedding1=nn.Embedding(128,embedding_dim)   #前一个指编码中的最大数，后一个指第三维是多少
        #卷积层，第一个是卷积核，然后最大池化，然后
        self.Cnn_model2=nn.Sequential(
            nn.Conv1d(in_channels=256,out_channels=kernel_number,kernel_size=2,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        self.Cnn_model3=nn.Sequential(
            nn.Conv1d(in_channels=256,out_channels=kernel_number,kernel_size=3,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        self.Cnn_model4=nn.Sequential(
            nn.Conv1d(in_channels=256,out_channels=kernel_number,kernel_size=4,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        self.Cnn_model5=nn.Sequential(
            nn.Conv1d(in_channels=256,out_channels=kernel_number,kernel_size=5,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        ...
        self.linear=nn.Linear(max_document_length*90,classfication)
    ...
    def forward(self, x):
        ...
        output, (h_n, c_n) = self.lstm1(x)  # 维度是(batch_size,seq_length,hidden_size*2) (64,48,256)
        tmp1 = self.linear1(output)    # (64,48,256)
        tmp2 = self.linear2(output)    # (64,48,256)
        tmp3 = self.linear3(output)    # (64,48,256)
        attn_output, attn_output_weights = self.attention(tmp1, tmp2, tmp3)  # (query,key,value) (64,48,256)  (64,48,48)
        ...
        x1 = self.Cnn_model2(attn_output)
        x2 = self.Cnn_model3(attn_output)
        x3 = self.Cnn_model4(attn_output)
        x4 = self.Cnn_model5(attn_output)
        x0 = torch.cat((x1, x2, x3, x4), dim=2)  # 这个地方是最终的输入维度（batch_size,max_document_length,90）
        output = self.flatten(x0)  # 维度是(batch_size,max_document_length*hidden_size) (64,4320)
