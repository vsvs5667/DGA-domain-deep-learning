...
class Reines(nn.Module):
    def __init__(self):
        ...
        self.Cnn_model2=nn.Sequential(
            nn.Conv1d(in_channels=30,out_channels=kernel_number,kernel_size=2,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        self.Cnn_model3=nn.Sequential(
            nn.Conv1d(in_channels=30,out_channels=kernel_number,kernel_size=3,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        self.Cnn_model4=nn.Sequential(
            nn.Conv1d(in_channels=30,out_channels=kernel_number,kernel_size=4,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        self.Cnn_model5=nn.Sequential(
            nn.Conv1d(in_channels=30,out_channels=kernel_number,kernel_size=5,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        ...
        self.linear=nn.Linear(max_document_length*346,classfication)
